...
CAzusa::CAzusa(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CCharacter(pDevice, pContext)
{
}

CAzusa::CAzusa(const CAzusa & rhs)
	:CCharacter(rhs)
{
}

HRESULT CAzusa::Init_Prototype()
{
...
HRESULT CAzusa::Init(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_TRANSLATION, XMVectorSet(24.f, 0.f, -2.f, 1.f));

	m_strObjName = TEXT("Azusa");
	return S_OK;
}

void CAzusa::Tick(_double TimeDelta)
{
	m_isRun = true;

	if (m_pCam->Get_CAMSTATE() == CCamera_Dynamic::STATE_TPS
		&& m_pCam->Get_CHARACTERCAM() == CCamera_Dynamic::CHARACTER_AZUSA)
	{
...
void CAzusa::Late_Tick(_double TimeDelta)
{
	__super::Late_Tick(TimeDelta);
}

HRESULT CAzusa::Render()
{
...
void CAzusa::Movement(_double TimeDelta)
{
...
void CAzusa::Collision_ToObstacle(_double TimeDelta)
{
	CGameInstance*			pGameInstance = GET_INSTANCE(CGameInstance);
...
void CAzusa::Collision_ToEnemy(_double TimeDelta)
{
	CGameInstance*			pGameInstance = GET_INSTANCE(CGameInstance);
...
void CAzusa::Imgui_RenderProperty()
{
	__super::Imgui_RenderProperty();
}

HRESULT CAzusa::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_CHAP1, TEXT("Prototype_Component_Model_Azusa"), TEXT("Com_Model"),
...
HRESULT CAzusa::SetUp_ShaderResources()
{
...
CAzusa * CAzusa::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CAzusa*		pInstance = new CAzusa(pDevice, pContext);

	if (FAILED(pInstance->Init_Prototype()))
	{
		MSG_BOX("Failed to Created : CAzusa");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CAzusa::Clone(void * pArg)
{
	CAzusa*		pInstance = new CAzusa(*this);

	if (FAILED(pInstance->Init(pArg)))
	{
		MSG_BOX("Failed to Cloned : CAzusa");
		Safe_Release(pInstance);
	}
	return pInstance;
}
