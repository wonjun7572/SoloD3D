...
HRESULT CShield::Init_Prototype()
{
...
HRESULT CShield::Init(void * pArg)
{
	if (nullptr != pArg)
		memcpy(&m_SheildDesc, pArg, sizeof(m_SheildDesc));
...
void CShield::Tick(_double TimeDelta)
{
...
void CShield::Late_Tick(_double TimeDelta)
{
	__super::Late_Tick(TimeDelta);

	_matrix			SocketMatrix =
		XMLoadFloat4x4(&m_SheildDesc.pSocket->Get_OffsetMatrix()) *
		XMLoadFloat4x4(&m_SheildDesc.pSocket->Get_CombindMatrix()) *
		XMLoadFloat4x4(&m_SheildDesc.PivotMatrix);

	SocketMatrix.r[0] = XMVector3Normalize(SocketMatrix.r[0]);
	SocketMatrix.r[1] = XMVector3Normalize(SocketMatrix.r[1]);
	SocketMatrix.r[2] = XMVector3Normalize(SocketMatrix.r[2]);

	SocketMatrix = SocketMatrix * m_SheildDesc.pTargetTransform->Get_WorldMatrix();
...
HRESULT CShield::SetUp_Components()
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_CHAP1, TEXT("Prototype_Component_Model_Shield"), TEXT("Com_Model"),
...
HRESULT CShield::SetUp_ShaderResources()
{
...
void CShield::Imgui_RenderProperty()
{
	ImGui::Begin("Weapon");
...
void CShield::Free()
{
	...
	if (true == m_bClone)
	{
		Safe_Release(m_SheildDesc.pSocket);
		Safe_Release(m_SheildDesc.pTargetTransform);
	}
