...
CTwister_Line::CTwister_Line(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CEffect_Mesh(pDevice, pContext)
{
}

CTwister_Line::CTwister_Line(const CTwister_Line & rhs)
	: CEffect_Mesh(rhs)
{
}

HRESULT CTwister_Line::Init_Prototype()
{
...
HRESULT CTwister_Line::Init(void * pArg)
{
...
void CTwister_Line::Tick(_double TimeDelta)
{
...
void CTwister_Line::Late_Tick(_double TimeDelta)
{
...
void CTwister_Line::Imgui_RenderProperty()
{
	__super::Imgui_RenderProperty();
}

HRESULT CTwister_Line::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_CHAP1, TEXT("Twister_Line"), TEXT("Com_Model"),
...
HRESULT CTwister_Line::SetUp_ShaderResources()
{
...
CTwister_Line * CTwister_Line::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTwister_Line*		pInstance = new CTwister_Line(pDevice, pContext);

	if (FAILED(pInstance->Init_Prototype()))
	{
		MSG_BOX("Failed to Created : CTwister_Line");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CTwister_Line::Clone(void * pArg)
{
	CTwister_Line*		pInstance = new CTwister_Line(*this);

	if (FAILED(pInstance->Init(pArg)))
	{
		MSG_BOX("Failed to Cloned : CTwister_Line");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CTwister_Line::Free()
{
	__super::Free();
}
