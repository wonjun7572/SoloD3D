...
CMonsterHpUI::CMonsterHpUI(const CEffect_Rect & rhs)
	:CEffect_Rect(rhs)
{
}

HRESULT CMonsterHpUI::Init_Prototype()
{
...
HRESULT CMonsterHpUI::Init(void * pArg)
{
...
void CMonsterHpUI::Tick(_double TimeDelta)
{
	...
	Compute_BillBoard();
	m_pTransformCom->Set_Scaled(_float3(1.8f, 0.5f, 1.f));
}
...
HRESULT CMonsterHpUI::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CMonsterHpUI::SetUp_ShaderResources()
{
	...
	if (FAILED(m_pTextureCom->Bind_ShaderResource(m_pShaderCom, "g_AlbedoTexture", 5)))
		return E_FAIL;

	if (FAILED(m_pTextureCom->Bind_ShaderResource(m_pShaderCom, "g_FillTexture", 3)))
		return E_FAIL;

	if (FAILED(m_pTextureCom->Bind_ShaderResource(m_pShaderCom, "g_BackTexture", 4)))
		return E_FAIL;
