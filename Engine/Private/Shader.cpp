...
HRESULT CShader::Init_Prototype(const wstring& pShaderFilePath, const D3D11_INPUT_ELEMENT_DESC* pElements, const _uint iNumElements)
{
	_uint		iHlslFlag = 0;
	...
	m_iNumPasses = TeniqueDesc.Passes;
...
CShader * CShader::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const wstring& pShaderFilePath, const D3D11_INPUT_ELEMENT_DESC* pElements, const _uint iNumElements)
{
	...
	if (FAILED(pInstance->Init_Prototype(pShaderFilePath, pElements, iNumElements)))
	{
